...
in
{
  ...
  config = lib.mkIf cfg.enable (
    let
      ...
      customKeybindings = lib.attrsets.listToAttrs (
        lib.lists.imap0 (index: customShortcut: {
    ...
    {
      dconf.settings = {
        "org/gnome/settings-daemon/plugins/media-keys".custom-keybindings = lib.mkIf (
          customKeybindingsRef != [ ]
        ) customKeybindingsRef;
      }
      // customKeybindings;
    }
