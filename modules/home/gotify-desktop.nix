...
in
{
    ...
    in
    {
      ...
      token = mkOption {
        type = types.oneOf [
          ...
          (types.submodule {
            options = {
              command = mkOption {
                type = types.str;
                example = "secret-tool lookup Title 'Gotify token'";
                description = ''
                  Command for fetching secret Gotify client token.
                '';
              };
            };
          })
        ...
        example = literalExpression ''
          {
            command = "secret-tool lookup Title 'Gotify token'";
          }
        '';
        description = ''
  ...
  config =
    ...
    lib.mkIf cfg.enable {
      ...
      # Based on the systemd service example provided by package
