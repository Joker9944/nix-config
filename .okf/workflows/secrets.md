...
* **Bootstrap order** (chicken/egg): a host can only decrypt after its SSH host key exists, and that must hold by the *first* activation, not the first boot. Until a host is added as a recipient the file is decryptable only by `joker9944`. [install-nixos](install-nixos.md) resolves this by generating the key ahead of the install and seeding it.
