...
export function formatDateTime(dateTime: GLib.DateTime, format = "%d. %b %H:%M"): string {
...
export function formatSpeed(speed: number): string {
	...
	if (speed < BIT_TO_KIBIBITS) {
		return `${speed.toFixed(DECIMAL_PLACES)}   b/s`.padStart(MAX_PADDING_LENGTH, " ")
	} else if (speed < BIT_TO_MEBIBITS) {
		return `${(speed / BIT_TO_KIBIBITS).toFixed(DECIMAL_PLACES)} Kib/s`.padStart(MAX_PADDING_LENGTH, " ")
	} else if (speed < BIT_TO_GIBIBITS) {
		return `${(speed / BIT_TO_MEBIBITS).toFixed(DECIMAL_PLACES)} Mib/s`.padStart(MAX_PADDING_LENGTH, " ")
	} else {
		return `${(speed / BIT_TO_GIBIBITS).toFixed(DECIMAL_PLACES)} Gib/s`.padStart(MAX_PADDING_LENGTH, " ")
	}
}
