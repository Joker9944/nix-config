			...
			<For each={filterWorkspaces(connector)}>
				{(workspace) => (
					<button onClicked={() => focusWorkspace(workspace)} cssClasses={workspaceButtonCssClasses(workspace)}>
						{isSpecial(workspace) ? (
							<image cssClasses={["font-2xl"]} iconName={specialWorkspaceName(workspace)!} />
						) : (
							<label cssName="value" cssClasses={["font-mono"]} label={workspace.get_name()} />
						)}
					</button>
				)}
			</For>
	...
	return focusedWorkspaceAccessor.as((focusedWorkspace) => {
		const focusClasses =
			...
			focusedWorkspace.get_id() === workspace.get_id()
				? ["selected"]
				: ["inactive"]
		return ["p-1", "m-0", "rounded-full", "min-width-8", "min-height-8"].concat(focusClasses)
	})
}
