...
description: Look up authoritative home-manager module options (name, type, default, example, description) from the JSON pinned by this flake. Use this skill whenever you are about to write, edit, or review a home-manager `.nix` module — anything touching `programs.*`, `services.*`, `wayland.*`, `home.*`, `xdg.*`, `gtk.*`, `qt.*`, `systemd.user.*`, or files under `modules/home/mixins/`, `modules/home/users/joker9944/config/`, `modules/home/users/*/hosts/*` — and for any question of the form "does home-manager support X" or "what's the option for Y". PREFER this over recalled knowledge and PREFER this over reading home-manager source files. Home-manager options get renamed and restructured between releases (e.g., `programs.direnv.enableNixDirenv` → `programs.direnv.nix-direnv.enable`, `programs.vscode.extensions` → `programs.vscode.profiles.<name>.extensions`), so training-data recall is a hallucination hazard. Also use it when a `nix build` fails with "The option ... does not exist", when you find yourself about to open a `<home-manager/modules/…>` source file to find an option, or when you're tempted to guess a submodule shape. It matters most right after a home-manager release bump, when recall, the `nix` MCP server, and every blog post still describe the release you just left.
---
...
The JSON is rebuilt from whatever `flake.lock` pins, so the moment home-manager is bumped it describes the *new* release while every other source still describes the old one. Mid-release recall and reality mostly agree and guessing is cheap; right after a bump they diverge and guessing is expensive. Reading module source doesn't settle it either, since an upgrade has two trees in play and it's easy to read the one you're leaving. See `.okf/workflows/release-upgrade.md`.
